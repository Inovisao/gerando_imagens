...
def process_and_augment_images(input_folder, output_folder, num_augmentations):
    ...
    augmenter = iaa.SomeOf((1, 5), [
        iaa.GaussianBlur(sigma=(0, 1.0)),
        iaa.LinearContrast((0.75, 1.5)),
        iaa.AdditiveGaussianNoise(scale=(0, 0.05 * 255)),
        iaa.Multiply((0.8, 1.2)),
        iaa.Affine(rotate=[90, 180]),  # Rotação apenas em 90 ou 180 graus
        iaa.MultiplyBrightness((0.8, 1.2)),  # Ajuste de iluminação
        iaa.LinearContrast((0.5, 2.0)),  # Ajuste de contraste
        iaa.AddToHueAndSaturation((-20, 20))  # Ajuste de saturação
    ], random_order=True)
